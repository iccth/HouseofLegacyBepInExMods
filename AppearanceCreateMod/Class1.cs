...
namespace AppearanceCreateMod
{
    ...
    public class AppearanceCreateMod : BaseUnityPlugin
    {
        ...
        //public static void HanMenMemberAdd_Postfix(string SexID)
        //{
        //	Log.LogInfo($"检测到郡城寒门子弟添加，当前寒门子弟总数: {Mainload.Member_Hanmen.Count}");
        //	if (!enableNewMemberHanMenOption.Value) return;
        //          try
        //          {
        ...
        //              // 正确获取寒门子弟数据
        //              List<string> newMemberData = Mainload.Member_Hanmen[lastHanMenIndex];
        ...
        //              if (newLiHui != null && personality != null)
        //              {
        //                  // 替换旧的立绘ID组合字符串 (根据MemberHanMenAdd方法源码，立绘在newMemberData索引1<第2个字符串>)
        //                  newMemberData[1] = newLiHui;

        //                  // 替换旧的品性ID (根据MemberHanMenAdd方法源码，品性ID（text3）在newMemberData索引2.8<第3个字符串第9个分割值>)
        //                  // 获取包含品性的字符串
        //                  string combinedInfo = newMemberData[2];
        //                  string[] infoArray = combinedInfo.Split('|');
        //                  infoArray[8] = personality;
        ...
        //                  // 将修改后的数据写回
        //                  Mainload.Member_Hanmen[lastHanMenIndex] = newMemberData;
        //                  if (enableNewMemberHanMenDebug.Value)
        //                  {
        //                      Log.LogInfo($"【郡城寒门子弟（可招募人员）生成】");
        //                      Log.LogInfo($"根据性别ID\"{sex}\"从自定义外观配置中生成随机立绘组合: \"{newLiHui}\" （后发|身体|脸型|前发）");
        //                      Log.LogInfo($"生成随机品性ID: \"{personality}\"");
        //                      Log.LogInfo($"该成员替换外观后的数据字符串为: \"{string.Join("|", newMemberData)}\"");
        //                  }
        ...
        //修改郡城人员面板寒门子弟立绘
        [HarmonyPostfix]
        [HarmonyPatch(typeof(FormulaData), "CreatHanMenMemberData", new Type[] { typeof(string), typeof(string), typeof(int) })]
        public static void CreatHanMenMemberData_Postfix(string SexID, ref List<string> __result)
		{
            //Log.LogInfo("检测到郡城寒门子弟生成");
            if (!enableNewMemberHanMenOption.Value) return;
            try
            {
				if (__result == null || __result.Count < 3) return;

				// 正确获取寒门子弟数据
				List<string> newMemberData = __result;
                ...
                if (newLiHui != null && personality != null)
                {
                    // 替换旧的立绘ID组合字符串 (根据CreatHanMenMemberData方法源码，立绘在newMemberData索引1<第2个字符串>)
                    newMemberData[1] = newLiHui;

                    // 替换旧的品性ID (根据CreatHanMenMemberData方法源码，品性ID（text3）在newMemberData索引2.8<第3个字符串第9个分割值>)
                    // 获取包含品性的字符串
                    string combinedInfo = newMemberData[2];
                    string[] infoArray = combinedInfo.Split('|');
                    infoArray[8] = personality;
